...
const createOrUpdateCourierService = async (req, res) => {
    ...
    if (req.body.id && req.body.id !== '0') {
        try {
            ...
            res.status(500).json({ status: 0, message: 'Something went wrong in updating courier service.', data: err.message });
        }
    } else {
        try {
            ...
            res.status(500).json({ status: 0, message: 'Something went wrong in inserting courier service.', data: err.message });
        }
    }
};
