...
ORDERS.courierboy_ServicesOrders = async (req, res) => {
    try {
        ...
        const courierBoyPostalCode = req.session.user.postal_code; // Fetch logged-in courier boy's postal code
        ...
        const filteredResult = result.filter(order => order.order_uid && order.order_uid.postal_code === courierBoyPostalCode);
        ...
        if (filteredResult.length > 0) {
            ...
            filteredResult.forEach((element, index) => {
                ...
                const sendOtpBtn = `<button class="btn btn-secondary send-otp-btn" data-email="${element.order_uid ? element.order_uid.email : ''}" data-order-id="${element._id}" data-toggle="modal" data-target="#otpModal">Send OTP</button>`;
...
ORDERS.sellerOrdersList = async (req, res) => {
    try {
        ...
        await orderProducts.find(query).populate('seller_id','fullname').populate('order_id','order_uniqueid payment_status payment_mode').populate('order_vid','pro_subtitle pro_sku').populate({ path: "order_uid", match: query  , select: "fullname " }) .populate('courier_service', 'service_name').sort({"group.nome": "asc"}).skip(start).limit(dataLimit).sort({ _id : 'desc' }).then(async (result)=>{
                ...
                result.forEach(function(element,key) {
                    //if(element.order_uid){
                        ...
                        mytable.data[key] = [checkBox+''+ ++start,
                                          ...
                                          payStatus,
                                          (element.order_id) ? element.order_id.payment_mode : '',
                                          (element.order_status == 1 || element.order_status == 5) ? (element.courier_service ? element.courier_service.service_name : 'N/A') : invoice + ' ' + lable + ' '+ track +' '+ returnBtn +' '+ refundBtn,
        ...
        if(orderDetails){
            ...
            if(orderStatus ==1){    //orderStatus = 1 for delivered
                ...
                await helper.deductInventory(orderDetails.order_vid, orderDetails.prod_quantity);
                    ...
                    await helper.deductSizeInventory(orderDetails.order_vid,orderDetails.prod_size,orderDetails.prod_quantity);
